...
export default function Projects() {
  return (
    ...
    >
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-lg text-muted">
            Some of these I wrote by hand. Others I built by steering AI tools.
            They're split up so you can see both.
          </p>
        </div>
            ...
            {groups.map((g) => (
              <li key={crypto.randomUUID()}>
                ...
                >
                  <span
                    aria-hidden="true"
                    className={`size-3 rounded-md ${g.badge}`}
                  />
                  {g.title}
      ...
      {groups.map((g) => (
        <div key={crypto.randomUUID()} id={g.id} className="mt-16">
          <div className="mb-8 flex flex-col gap-2 border-t border-line pt-8 sm:flex-row sm:items-baseline sm:gap-6">
            <h3 className="flex shrink-0 items-center gap-3 text-3xl font-medium">
              <span
                aria-hidden="true"
                className={`size-4 rounded-md ${g.badge}`}
              />
              {g.title}
            </h3>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {g.items.map((p) => (
              <ProjectCard key={crypto.randomUUID()} project={p} />
            ))}
          </div>
        </div>
      ))}
